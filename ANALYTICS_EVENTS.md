...
## События Воронки Пополнения

Эти события предназначены для отслеживания пути пользователя в процессе пополнения баланса.

### 22. `topup_start`

*   **Когда происходит:** Пользователь начинает процесс пополнения баланса (команда `/topup` или кнопка "Пополнить баланс").
*   **Описание:** Логирует начало воронки пополнения баланса.
*   **Полезная нагрузка (`event_data`):**
    ```json
    {}
    ```
...
### 23. `topup_package_selected`

*   **Когда происходит:** Пользователь выбирает пакет для пополнения.
*   **Описание:** Логирует выбор пакета пополнения.
*   **Полезная нагрузка (`event_data`):**
    ```json
    {
      "package": {
    ...
    ```
    *   `package`: JSON-объект с информацией о выбранном пакете.

---

### 24. `topup_method_selected`

*   **Когда происходит:** Пользователь выбирает метод оплаты.
*   **Описание:** Логирует выбор метода оплаты.
*   **Полезная нагрузка (`event_data`):**
    ```json
    {
