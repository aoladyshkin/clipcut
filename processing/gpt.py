...
Длина каждого клипа: от {format_seconds_to_hhmmss(MIN_SHORT_DURATION)} до {format_seconds_to_hhmmss(MAX_SHORT_DURATION)}.
Оптимальная длина: {int(MIN_SHORT_DURATION + (MAX_SHORT_DURATION - MIN_SHORT_DURATION) / 2)}–{int(MAX_SHORT_DURATION)} секунд.
Клип должен быть понятен без контекста всего интервью.
Если потенциальный клип получился <{int(MIN_SHORT_DURATION)} секунд, обязательно расширь его за счёт соседних реплик (вперёд или назад), сохранив смысловую цельность.
...
2.  **Строго соблюдай длительность.** Минимальная длина — {int(MIN_SHORT_DURATION)} секунд, максимальная — {int(MAX_SHORT_DURATION)} секунд. Клипы за пределами этого диапазона будут отброшены.
...
Убедись, что каждый клип дольше {int(MIN_SHORT_DURATION)} секунд.
ВЫВЕДИ ТОЛЬКО JSON.
''')
    return prompt
